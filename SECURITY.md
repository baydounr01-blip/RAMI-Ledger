...
| **Perfil del jugador** (`SetProfile`, v0.10.0) | suplantación de un avatar, acaparamiento de nombres, vínculo de un nodo ajeno | el vínculo cuenta ↔ nodo exige la **firma Ed25519 de la identidad del nodo** sobre la cuenta y el consenso la verifica (`verify_strict`); un nodo solo puede estar vinculado a una cuenta; nombre único con alfabeto fijo (`a-z`, `0-9`, `_`, sin normalización Unicode) y **2 RAMI quemados** por registro o cambio; alias y bio acotados en bytes y UTF-8 válido; el nombre declarado en la presencia nunca sustituye al del perfil (solo el vínculo verificado pone ✓) |
